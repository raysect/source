...
Running this demo therefore requires an MPI library, such as MPICH or
OpenMPI. The mpi4py Python package must also be installed.
...
from raysect.core.workflow import MulticoreEngine, HybridEngine
...
# Work a bit harder across a cluster.
camera.pixels = (512, 512)
camera.pixel_samples = 500

# Either hard-code the number of sub-engine workers, or infer it from the run
# time environment if using a supported job scheduler like Slurm or Grid Engine.
# nworkers = 8
nworkers = HybridEngine.estimate_subworker_count()
camera.render_engine = HybridEngine(MulticoreEngine(nworkers))
print(f"MPI process {camera.render_engine.name} running with {nworkers} workers.")

# With the hybrid engine, only the rank 0 process has all the results available
