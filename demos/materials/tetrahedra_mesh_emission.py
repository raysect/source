...
mesh = Tetmesh(os.path.join(BASE_PATH, "../resources/pig_tet.mesh"))
...
floor = Box(Point3D(-100, -0.3, -100), Point3D(100, -0.2, 100), world, material=RoughTitanium(0.1))

# camera
camera_pos = Point3D(-0.6, 0.25, -1.1)
rgb_pipeline = RGBPipeline2D(display_update_time=5)
sampler = RGBAdaptiveSampler2D(rgb_pipeline, min_samples=100, fraction=0.2)
camera = PinholeCamera((512, 512), parent=world, transform=translate(camera_pos.x, camera_pos.y, camera_pos.z) * rotate_basis(camera_pos.vector_to(Point3D(0, 0, 0)), Vector3D(0, 1, 0)), pipelines=[rgb_pipeline], frame_sampler=sampler)
camera.fov = 50
...
emitter.material.integrator.step = 0.005
