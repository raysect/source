...
def equilateral_prism(width=1.0, height=1.0, parent=None, transform=None, material=None):
    ...
    centre = Box(Point3D(-half_width, 0, 0), Point3D(half_width, height, width))
    left = Box(Point3D(0, -0.001, -0.001), Point3D(width, height + 0.001, 2 * width), transform=translate(half_width, 0, 0) * rotate(30, 0, 0))
    right = Box(Point3D(-width, -0.001, -0.001), Point3D(0.0, height + 0.001, 2 * width), transform=translate(-half_width, 0, 0) * rotate(-30, 0, 0))
...
prism = equilateral_prism(0.05, 0.15, parent=world, material=schott("SF11"), transform=translate(0, 0.0 + 1e-6, 0))
...
lightbox = light_box(parent=world, transform=rotate(-35.5, 0, 0) * translate(0.10, 0, 0) * rotate(90, 0, 0))
